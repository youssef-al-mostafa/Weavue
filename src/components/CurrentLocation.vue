...
const errorMessage = ref({
  msg: "Error while fetching your location. Please try again later.",
  active: false
});
  ...
  if ("geolocation" in navigator) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        errorMessage.value.msg = `Location error: ${error.message}`;
...
<template>
  <div class="current-location-cont mb-2">
    <button class="btn bg-transparent border-none 
                   font-sans w-full text-nowrap 
                   shadow-none flex justify-start 
                   gap-4 text-[18px] text-yellow-600" @click="goWithCurrentLocation">
      <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
        x="0px" y="0px" width="24px" height="24px" viewBox="0 0 120.87 122.88"
        style="enable-background:new 0 0 120.87 122.88" xml:space="preserve">
        <g>
          <path
  ...
  <div v-if="errorMessage.active" role="alert" class="alert alert-error">
